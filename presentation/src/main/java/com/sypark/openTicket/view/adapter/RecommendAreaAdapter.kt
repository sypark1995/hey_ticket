...
class RecommendAreaAdapter : ListAdapter<Areas, RecommendAreaAdapter.ViewHolder>(DiffCallback()) {
    ...
    class DiffCallback : DiffUtil.ItemCallback<Areas>() {
        override fun areItemsTheSame(oldItem: Areas, newItem: Areas): Boolean = oldItem.code == newItem.code
        override fun areContentsTheSame(oldItem: Areas, newItem: Areas): Boolean = oldItem == newItem
    }
}
